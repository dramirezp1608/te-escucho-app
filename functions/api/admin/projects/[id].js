...
export async function onRequestPatch(context) {
    try {
        ...
        const result = await dataverseRequest(context.env, `coem_proyectoinnovacions(${id})`, 'PATCH', body);
...
export async function onRequestDelete(context) {
    try {
        await verifyAdmin(context.request, context.env);
        const id = context.params.id;
        await dataverseRequest(context.env, `coem_proyectoinnovacions(${id})`, 'DELETE');
